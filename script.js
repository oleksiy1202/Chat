...
function login() {
    ...
    if (currentUser) {
        if (!users[currentUser]) {
            ...
            sendSystemMessage(`${currentUser} приєднався до чату о ${getCurrentTime()}.`, 'blue')
        }
...
function logout() {
    sendSystemMessage(`${currentUser} покинув чат о ${getCurrentTime()}.`, 'red')
    currentUser = ''
...
function sendSystemMessage(text, color) {
    const messageContainer = document.createElement('div')
    messageContainer.classList.add('message')
    ...
    const textSpan = document.createElement('span')
    textSpan.classList.add('text')
    textSpan.innerHTML = `<span style="color: ${color};">${text}</span>`
